...
const EditEmConPhoneModalContractor = ({ currentContractor, onContractorChange, info }) => {
  ...
  async function editEmConPhone({ emConPhone }) {
    try {
      const ContractorDetails = {
        id: info.id,
        _version: info._version,
        emergency_contact_phone: emConPhone
      }
  ...
  return (
    <>
      ...
      <Modal initialFocusRef={initialRef} isOpen={isOpen} onClose={onClose}>
        <ModalOverlay />
        <ModalContent as="form" onSubmit={handleSubmit(editEmConPhone)} backgroundColor="#020202">
          ...
          <ModalBody pb={6}>
            <FormControl>
              ...
              <Input
                placeholder={info.emergency_contact_phone}
                {...register('emConPhone', { required: true })}
...
export default EditEmConPhoneModalContractor
