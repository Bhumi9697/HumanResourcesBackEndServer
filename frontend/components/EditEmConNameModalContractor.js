...
const EditEmConNameModalContractor = ({ currentContractor, onContractorChange, info }) => {
  ...
  async function editEmConName({ emConName }) {
    try {
      const ContractorDetails = {
        id: info.id,
        _version: info._version,
        emergency_contact_name: emConName
      }
  ...
  return (
    <>
      ...
      <Modal initialFocusRef={initialRef} isOpen={isOpen} onClose={onClose}>
        <ModalOverlay />
        <ModalContent as="form" onSubmit={handleSubmit(editEmConName)} backgroundColor="#020202">
          ...
          <ModalBody pb={6}>
            <FormControl>
              ...
              <Input
                placeholder={info.emergency_contact_name}
                {...register('emConName', { required: true })}
...
export default EditEmConNameModalContractor
