...
export default function ImageDropzoneListCContractor({ employeeId, employeeVersion, name }) {
  ...
  async function uploadImage() {
    try {
      const fileName = 'listC-image' + ' ' + name
      await Storage.put(fileName, files[0])
      const employeeDetails = {
        id: employeeId,
        ListC_image: fileName,
      ...
      toast({
        title: 'Successfully uploaded List C Document',
        status: 'success',
        duration: 2000,
        isClosable: true
      })
    ...
    <>
      <section className="container">
        <div {...getRootProps({ className: 'dropzone', style })}>
          <input {...getInputProps()} />
          <p>Drag and drop List C file here, or click to select file. Format MUST be .PNG, .JPG, or .JPEG</p>
        </div>
