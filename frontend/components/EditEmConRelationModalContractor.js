...
const EditEmConRelationModalContractor = ({ currentContractor, onContractorChange, info }) => {
  ...
  async function editEmConRelation({ emConRelation }) {
    try {
      const ContractorDetails = {
        id: info.id,
        _version: info._version,
        emergency_contact_relation: emConRelation
      }
  ...
  return (
    <>
      ...
      <Modal initialFocusRef={initialRef} isOpen={isOpen} onClose={onClose}>
        <ModalOverlay />
        <ModalContent as="form" onSubmit={handleSubmit(editEmConRelation)} backgroundColor="#020202">
          <ModalHeader color="#F5F5F5" fontSize="2xl" fontWeight="normal" textAlign="center">
            Edit Emergency Contact Relation
          </ModalHeader>
          ...
          <ModalBody pb={6}>
            <FormControl>
              ...
              <Input
                placeholder={info.emergency_contact_relation}
                {...register('emConRelation', { required: true })}
...
export default EditEmConRelationModalContractor
